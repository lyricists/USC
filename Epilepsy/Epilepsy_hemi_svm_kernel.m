...
d = 0.02;
[x1Grid,x2Grid] = meshgrid(-3:d:3, -4:d:4);
xGrid = [x1Grid(:), x2Grid(:)];
...
concord_mvp = (length(find((class-dc_mvp) == 0))/34)*100;
...
maxv = (max([max_left; max_right]));

max_left = max_left./maxv;
max_right = max_right./maxv;
pix = (pix_left - pix_right)./max(abs(pix_left - pix_right));
